...
| **Language / runtime** | Go, Python, TypeScript | **Go 1.22+** | First-class concurrency (the worker model is goroutines + a consumer group), static binaries that make tiny container images, strong typing for the data contracts, and a fast test story with the race detector. |
...
- **Reconciler:** move from the API process to its own deployment once the API runs many replicas.
